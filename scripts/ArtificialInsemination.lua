...
-- FS25 does not expose the <reproduction> XML values in the same nested table
-- on every game version. Keep the XML defaults as a fallback so eligible
-- animals are not rejected just because that runtime table is missing.
...
local function getReproductionRule(subType)
    ...
    if string.find(name, "ROOSTER", 1, true) ~= nil then return nil end

    local reproduction = subType ~= nil and subType.reproduction or nil
    if reproduction ~= nil and reproduction.supported == false then return nil end
    ...
    -- Compatibility with game versions that flatten the reproduction values.
...
-- Returns the remaining gestation time in in-game months for one individual animal.
-- The calculation uses the exact same reproduction duration as the birth logic.
...
-- Shared by both lists; maturity is independent of an existing pregnancy.
...
function ArtificialInsemination.install()
    ...
    AnimalCluster.updateReproduction = function(cluster, ...)
        -- Existing pregnancies from an older savegame may finish. New pregnancies
        -- are managed individually by this mod and never started automatically.
        local _, chicken = VeterinarianLife.rule(getSubType(cluster))
        if chicken or (cluster.reproduction or 0) > 0 then return originalUpdateReproduction(cluster, ...) end
        return 0
    end

    -- The global PERIOD_CHANGED message is not emitted consistently for mods
    -- in FS25. Every animal husbandry does receive its own period callback,
    -- so advance our individual pregnancies there instead.
...
function ArtificialInsemination.perform(husbandry, animalIndex, connection)
...
function ArtificialInsemination.request(husbandry, animalIndex)
    if g_server ~= nil then
        ArtificialInsemination.perform(husbandry, animalIndex, nil)
    elseif g_client ~= nil then
        g_client:getServerConnection():sendEvent(ArtificialInseminationEvent.new(husbandry, animalIndex))
    end
end
...
function ArtificialInsemination.onHusbandryPeriodChanged(husbandry)
    ...
    for animalIndex, progress in pairs(state.inseminatedAnimals) do
        ...
        if chicken then
            ...
            if nextProgress >= duration then
                ...
                -- A completed birth starts a 12-month lactation for cows/goats,
                -- even if the barn is full and some/all newborns are sold.
                ...
                if sold > 0 then
                    ...
                    g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_INFO,
                        string.format(g_i18n:getText("vet_birthAutoSold"), sold, g_i18n:formatMoney(salePrice, 0, true, true)))
                end

                g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_INFO,
                    string.format(g_i18n:getText("vet_birthResult"), state.animalNames[animalIndex], alive, extra.litter[animalIndex] - alive))
                state.inseminatedAnimals[animalIndex] = nil
                state.inseminatedSubTypes[animalIndex] = nil
                extra.litter[animalIndex] = nil
                extra.stillRate[animalIndex] = nil
    ...
    local snapshot = VeterinarianLife.snapshot(husbandry, state)
    for _, birth in ipairs(births) do husbandry:addAnimals(birth[1], birth[2], 0) end
    if #births > 0 then
        husbandry.spec_husbandryAnimals.clusterSystem:updateNow()
        VeterinarianLife.restore(husbandry, state, snapshot)
    end
...
function ArtificialInsemination.periodChanged()
    -- Compatibility entry point for older saves/game versions. The active FS25
    -- path is onHusbandryPeriodChanged, installed above.
